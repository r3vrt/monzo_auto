...
class AutoTopup:
    ...
    def _should_trigger_topup(self, rule: TopupRule) -> bool:
        ...
        if rule.trigger_type == "monthly":
            ...
            should_trigger = today.day == (rule.trigger_day or 1)
            logger.info(f"📅 Monthly trigger for rule '{rule.name}': current day {today.day}, trigger day {rule.trigger_day or 1}, should trigger: {should_trigger}")
            return should_trigger

        elif rule.trigger_type == "weekly":
            today = datetime.now(timezone.utc)
            should_trigger = today.weekday() == (rule.trigger_day or 0)  # Monday = 0
            logger.info(f"📅 Weekly trigger for rule '{rule.name}': current weekday {today.weekday()}, trigger day {rule.trigger_day or 0}, should trigger: {should_trigger}")
            return should_trigger

        elif rule.trigger_type == "daily":
            today = datetime.now(timezone.utc)
            should_trigger = today.hour == (rule.trigger_hour or 0) and today.minute == (rule.trigger_minute or 0)
            logger.info(f"📅 Daily trigger for rule '{rule.name}': current time {today.hour:02d}:{today.minute:02d}, trigger time {rule.trigger_hour or 0:02d}:{rule.trigger_minute or 0:02d}, should trigger: {should_trigger}")
            return should_trigger

        elif rule.trigger_type == "hourly":
            today = datetime.now(timezone.utc)
            should_trigger = today.minute == (rule.trigger_minute or 0)
            logger.info(f"📅 Hourly trigger for rule '{rule.name}': current minute {today.minute}, trigger minute {rule.trigger_minute or 0}, should trigger: {should_trigger}")
            return should_trigger
            ...
            # If target_balance is set, also check balance threshold
            balance_trigger = False
            if rule.target_balance is not None:
                logger.info(f"🎯 Checking balance threshold for rule '{rule.name}' with target balance {rule.target_balance} ({rule.target_balance/100:.2f}£)")
                current_balance = self._get_account_balance(rule.target_pot_id)
                if current_balance is not None:
                    balance_trigger = current_balance < rule.target_balance
                    logger.info(f"💰 Balance threshold check: current={current_balance} ({current_balance/100:.2f}£), target={rule.target_balance} ({rule.target_balance/100:.2f}£), balance trigger: {balance_trigger}")
                else:
                    logger.error(f"❌ Could not get balance for target {rule.target_pot_id}")
